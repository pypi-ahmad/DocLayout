"""Normalize DocLayout markdown for olmOCR-bench scoring (output-only).
...
Net effect is small (DocLayout's markdown is already fairly clean): on olmOCR-bench
...
def postprocess(md: str) -> str:
    """Map DocLayout markdown to olmOCR-bench-friendly text (output-only)."""
    s = re.sub(
        r"<math\b[^>]*>(.*?)</math>",
